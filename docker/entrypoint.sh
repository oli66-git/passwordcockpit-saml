...
if [ $count -gt "2" ]; then
        echo -e "\e[31mWARNING: $PWD is not empty! Passwordcockpit will not be installed\e[0m" >&2
else
    # move passwordcockpit source in container
    shopt -s dotglob
    mv /usr/src/passwordcockpit/* ./
    echo -e "\e[32mSource copied in $PWD\e[0m" >&2
    ...
    echo -e "\e[32mConfiguration files created and modified\e[0m" >&2

    ##############################################
    # Database
    ##############################################
    echo -e "\e[31mCheck database connection\e[0m" >&2
    max_retries=10
    try=0
    ...
    do
        ...
        if [ -z "${connection}" ]; then
            echo -e "\e[31mRetrying connection...\e[0m" >&2
            try=$((try+1))
            sleep 3s
            continue
        fi
        ...
        if [ "$schema_exist" == "1" ]; then
            echo -e "\e[32mConnection ok\e[0m" >&2
            echo -e "\e[32mSchema already exist\e[0m" >&2
            # Tables exists
            ...
            if [ "$number_of_tables" == "0" ]; then
                ...
                echo -e "\e[32mDatabase created\e[0m" >&2
                sql=$(cat database/create-tests-environment.sql | sed '/^--/d')
                vendor/bin/doctrine dbal:run-sql "$sql"
                echo -e "\e[32mTest data installed\e[0m" >&2
            else
                # Update scripts
                echo -e "\e[32mNo updates to be carried out\e[0m" >&2
            fi
            break
        fi
        ...
        if [ "$schema_exist" == "0" ]; then
            echo -e "\e[32mConnection ok\e[0m" >&2
            echo -e "\e[31mSchema not exist\e[0m" >&2
            echo -e "\e[31mInstalling failed!\e[0m" >&2
            exit 1
            break
        fi
    done
    ...
    echo -e "\e[32mPasswordcockpit ready\e[0m" >&2
